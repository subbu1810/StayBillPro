...
export default function SuppliersScreen({ defaultTab }) {
    ...
    const handleDownloadReceipt = (payment) => {
        ...
        doc.setFontSize(12);
        doc.text(`Receipt No: ${payment.reference_no || 'N/A'}`, 14, 40);
        doc.text(`Date: ${new Date(payment.payment_date).toLocaleDateString()}`, 14, 48);
        
        doc.text(`Paid To: ${payment.supplier_name}`, 14, 60);
        doc.text(`Amount: Rs. ${Number(payment.amount).toLocaleString()}`, 14, 68);
        doc.text(`Payment Mode: ${payment.payment_method}`, 14, 76);
        doc.text(`Notes: ${payment.notes || '-'}`, 14, 84);

        doc.setFontSize(10);
        doc.text("Generated by StayBillPro", 105, 120, null, null, "center");

        doc.save(`receipt_${payment.supplier_name.replace(/\s+/g, '_')}_${new Date(payment.payment_date).getTime()}.pdf`);
    };
